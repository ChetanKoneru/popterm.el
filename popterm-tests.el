...
(ert-deftest popterm-test-frame-descendant-p-detects-nested-child-frame ()
  "Verify child-frame ancestry detection follows nested parent frames."
  (cl-letf (((symbol-function 'framep)
             (lambda (frame) (memq frame '(root popterm minibuffer unrelated))))
            ...
            ((symbol-function 'frame-parent)
             (lambda (frame)
               (pcase frame
                 ...
                 ('popterm 'root)
                 (_ nil)))))
    (should (popterm--frame-descendant-p 'minibuffer 'popterm))
    (should (popterm--frame-descendant-p 'minibuffer 'root))
    (should-not (popterm--frame-descendant-p 'popterm 'minibuffer))
    (should-not (popterm--frame-descendant-p 'unrelated 'popterm))))

(ert-deftest popterm-test-delete-descendant-minibuffer-posframes-removes-stale-nested-frame ()
  "Verify stale minibuffer posframes below Popterm are deleted."
  (let ((popterm--frame 'popterm)
        (deleted-frames nil)
        (stale-buffer (get-buffer-create " *popterm-test-stale-minibuf*"))
        (regular-buffer (get-buffer-create "*popterm-test-regular*")))
    (unwind-protect
        (cl-letf (((symbol-function 'framep)
                   (lambda (frame) (memq frame '(root popterm stale regular))))
                  ((symbol-function 'frame-live-p)
                   (lambda (frame) (memq frame '(root popterm stale regular))))
                  ((symbol-function 'frame-parent)
                   (lambda (frame)
                  ...
                  ((symbol-function 'frame-list)
                   (lambda () '(stale regular popterm root)))
                  ((symbol-function 'frame-parameter)
                   (lambda (frame parameter)
                     (when (eq parameter 'posframe-buffer)
                       (pcase frame
                         ('stale (cons (buffer-name stale-buffer) stale-buffer))
                         ('regular (cons (buffer-name regular-buffer) regular-buffer))))))
                  ((symbol-function 'active-minibuffer-window)
                   (lambda () nil))
                  ((symbol-function 'minibufferp)
                   (lambda (buffer) (eq buffer stale-buffer)))
                  ((symbol-function 'delete-frame)
                   (lambda (frame &optional _force)
                     (push frame deleted-frames))))
          (popterm--delete-descendant-minibuffer-posframes)
          (should (equal deleted-frames '(stale))))
      (when (buffer-live-p stale-buffer)
        (kill-buffer stale-buffer))
      (when (buffer-live-p regular-buffer)
        (kill-buffer regular-buffer)))))

(ert-deftest popterm-test-delete-descendant-minibuffer-posframes-preserves-active-minibuffer ()
  "Verify active nested minibuffer posframes are not deleted mid-completion."
  (let* ((popterm--frame 'popterm)
         (deleted-frames nil)
         (active-window (selected-window))
         (active-buffer (window-buffer active-window)))
    (cl-letf (((symbol-function 'framep)
               (lambda (frame) (memq frame '(root popterm active))))
              ((symbol-function 'frame-live-p)
               (lambda (frame) (memq frame '(root popterm active))))
              ((symbol-function 'frame-parent)
               (lambda (frame)
                 (pcase frame
                   ...
                   (_ nil))))
              ((symbol-function 'frame-list)
               (lambda () '(active popterm root)))
              ((symbol-function 'frame-parameter)
               (lambda (frame parameter)
                 (when (and (eq frame 'active)
                            (eq parameter 'posframe-buffer))
                   (cons (buffer-name active-buffer) active-buffer))))
              ((symbol-function 'active-minibuffer-window)
               (lambda () active-window))
              ((symbol-function 'minibufferp)
               (lambda (buffer) (eq buffer active-buffer)))
              ((symbol-function 'delete-frame)
               (lambda (frame &optional _force)
                 (push frame deleted-frames))))
      (popterm--delete-descendant-minibuffer-posframes)
      (should-not deleted-frames))))

(ert-deftest popterm-test-dismiss-descendant-posframes-hides-which-key ()
  "Verify quitting Ghostel hides a which-key posframe nested below Popterm."
  (let ((popterm--frame 'popterm)
        (which-key-buffer (get-buffer-create " *which-key-test*"))
        (hidden-buffers nil))
    (unwind-protect
        (cl-letf (((symbol-function 'framep)
                   (lambda (frame) (memq frame '(root popterm which-key))))
                  ((symbol-function 'frame-live-p)
                   (lambda (frame) (memq frame '(root popterm which-key))))
                  ((symbol-function 'frame-visible-p)
                   (lambda (frame) (eq frame 'which-key)))
                  ((symbol-function 'frame-parent)
                   (lambda (frame)
                     (pcase frame
                       ('which-key 'popterm)
                       ...
                       (_ nil))))
                  ((symbol-function 'frame-list)
                   (lambda () '(which-key popterm root)))
                  ((symbol-function 'frame-parameter)
                   (lambda (frame parameter)
                     (when (and (eq frame 'which-key)
                                (eq parameter 'posframe-buffer))
                       (cons (buffer-name which-key-buffer)
                             which-key-buffer))))
                  ((symbol-function 'posframe-hide)
                   (lambda (buffer)
                     (push buffer hidden-buffers))))
          (popterm--dismiss-descendant-posframes)
          (should (equal hidden-buffers (list which-key-buffer))))
      (when (buffer-live-p which-key-buffer)
        (kill-buffer which-key-buffer)))))

(ert-deftest popterm-test-ghostel-quit-dismisses-posframes-and-sends-C-g ()
  "Verify Popterm's Ghostel quit cleanup preserves terminal interrupt input."
  (let ((events nil))
    (cl-letf (((symbol-function 'popterm--dismiss-descendant-posframes)
               (lambda () (push 'dismiss events)))
              ((symbol-function 'ghostel-send-C-g)
               (lambda () (interactive) (push 'send events))))
      (popterm--ghostel-send-C-g)
      (should (equal (nreverse events) '(dismiss send))))))
